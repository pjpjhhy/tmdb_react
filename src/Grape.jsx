...
const[page,setPage] = useState(1);
  ...
  useEffect(()=>{
    ...
    const url = `https://api.themoviedb.org/3/tv/popular?language=ko&page=${page}`;
    const options = {
    method: 'GET',
    headers: {
    ...
    <Layout>
      <div className='w-full flex flex-col items-center justify-center py-16 '>
        <div className='w-[1000px] flex flex-wrap gap-4'>
          ...
          <div key={list.id} className='w-[180px] h-[340px] bg-blue-200 overflow-hidden rounded-lg shadow-xl'>
            {/* top: image */}
            <div className='w-full h-[250px] flex justify-center items-center '>
                {list.poster_path ? (
                <Link to={`/info/${list.id}`} state={'tv'} key={list.id}>
                <img
            ...
            <div className='relative w-full h-[90px] text-black pt-4 px-2'>
              <h2 className='font-bold text-lg '>{list.name?.substr(0,24)}</h2>
              <p className='text-sm '>{list.first_air_date}</p>
              {/* like  */}
